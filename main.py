...
#Configure your application for login(s) & logout(s)
login_manager = LoginManager()
login_manager.init_app(app)

#Initialize Flask App with Gravatar
...
def admin_only(function):
    ...
    def wrapper_function(*args, **kwargs):
        #If id is not one then return abort with 403(forbidden) error
...
def register():
    ...
    if user_registration_form.validate_on_submit():
        ...
        if Users.query.filter_by(email=user_registration_form.email.data).first():
            # Send flash messsage
    ...
    return render_template("register.html", form=user_registration_form, current_user=current_user)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, login_form.password.data):
            ...
            flash("This email does not exist, Register first !")
            return redirect(url_for("login"))
        else:
            flash("Password entered is incorrect, try again !")
            return redirect(url_for("login"))
...
if __name__ == "__main__":
    app.run(debug=os.getenv('debug', default=bool))
